...
class BasePage(CloneMixin, PolymorphicMPTTModel, PageLockViewMixin):
    ...
    draft_data = models.JSONField(
        null=True,
        blank=True,
        verbose_name='Данные черновика',
        help_text='JSON с данными черновика страницы и компонентов'
    )
    ...
    def get_live_instance(self):
        """Возвращает опубликованную (оригинальную) страницу для данного черновика, иначе self."""
        if self.is_draft and self.draft_parent_id:
            return self.draft_parent.get_real_instance() or self.draft_parent
        return self
