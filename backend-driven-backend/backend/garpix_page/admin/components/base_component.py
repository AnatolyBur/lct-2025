...
class BaseComponentAdmin(PolymorphicChildModelAdmin, TabbedTranslationAdmin, CreateLogMixin):
    ...
    # Поля черновиков больше не используются
...
class RealBaseComponentAdmin(PolymorphicParentModelAdmin, TabbedTranslationAdmin, CreateLogMixin):
    # Разрешаем открывать change-view даже для записей базовой модели,
    # которые по ошибке были созданы без конкретного подтипа
    child_models = [BaseComponent] + get_garpix_page_component_models()
    base_model = BaseComponent
    ...
    def _get_real_admin_by_model(self, model_class, super_if_self=False):
        """
        Переопределяем метод для обработки базовой модели BaseComponent.
        Если модель является базовой BaseComponent, возвращаем BaseComponentAdmin.
        """
        if model_class == BaseComponent:
            return BaseComponentAdmin(self.model, self.admin_site)
        return super()._get_real_admin_by_model(model_class, super_if_self)
